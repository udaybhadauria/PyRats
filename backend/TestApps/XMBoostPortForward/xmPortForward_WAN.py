...
def set_blob(mac_address, subdoc, json):
    ...
    if response_code == 200 and "POST Request Successful" in response_body:
        print("Successfully added the portForward Rule")
        return True
    else:
        print("Failed to set the PortForwarding Rule")
        return False

def fetch_row_id(mac_address, subdoc):
    row_id = None
    ...
    if response_code == 200:
        print("Successfully get the portForwarding Rule")
        response_json = json.loads(response_body)
        response_data = response_json['output'].split('Response = ')
        json_data = json.loads(response_data[1])
        data = json_data['data']
        for item in data:
            if item['name'] == 'RATSxmshhTest':
                row_id = item['row_id']
                break
        if not row_id:
            print("No matching row_id found")
            return False, row_id
        return True, row_id
    else:
        print("Failed to get the PortForwarding Rule")
        return False, row_id

def delete_blob(mac_address, subdoc, group):
    ...
    if response_code == 200 and "DELETE Request Successful" in response_body:
        ...
        print("Failed to delete the Portforwarding rule added")
        return False

if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python3 xmPortForward_WAN.py <test_ID> <mac_address> <lanclient_ip> <gateway_wan_ip>")
        sys.exit(1)
    ...
    #---------------------------- Case 1: Validate the PortForwarding with speedboost port range 40001 - 41000 -----------------

    print("------Validate the PortForwarding with speedboost port range 40001 - 41000----")
    ...
    if not response:
        result = "Failed"
        description = "Failed to set PortForwarding Rule"
        write_test_result_to_json(mac_address, test_ID, result, description)
        webpa_set(mac_address, pvdParameter, pvdEnable, 3)
        webpa_set(mac_address, xmPortParameter, defaultXmport, 0)
    ...
    response, row_id = fetch_row_id(mac_address, subdoc)
    if not response:
        result = "Failed"
        description = "Failed to fetch the row_id of the portforward rule added"
        write_test_result_to_json(mac_address, test_ID, result, description)
        webpa_set(mac_address, pvdParameter, pvdEnable, 3)
        webpa_set(mac_address, xmPortParameter, defaultXmport, 0)
        webpa_set(mac_address, normalPortParameter, defaultNormalPort, 0)
    ...
    response = delete_blob(mac_address, subdoc, group)
    if not response:
        result = "Failed"
        description = "Failed to delete the portforward rule added"
        write_test_result_to_json(mac_address, test_ID, result, description)
        webpa_set(mac_address, pvdParameter, pvdEnable, 3)
        webpa_set(mac_address, xmPortParameter, defaultXmport, 0)
        webpa_set(mac_address, normalPortParameter, defaultNormalPort, 0)
    ...
    #---------------------------- Case 2: Validate the PortForwarding with speedboost port range 45001 - 46000 -----------------

    print("------Validate the PortForwarding with speedboost port range 45001 - 46000----")
    # Disable speedboost feature
    print("Disable speedboost feature")
    response = webpa_set(mac_address, pvdParameter, False, 3)
    if not response:
