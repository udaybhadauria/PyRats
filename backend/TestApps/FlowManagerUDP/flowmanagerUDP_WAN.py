...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 flowManagerUDP_WAN.py <test_ID> <mac_address> <gateway_wan_ip>")
        sys.exit(1)
    ...
    filter_exp = f"ip src {gateway_wan_ip} and udp src port 22223 and udp dst port {PORT}"
    ...
    try:
        received_any_data = False  # Track if any data was received
        ...
        if not received_any_data:
            print("No connection received within 2 minutes. Closing server.")
        else:
            print("No further data received. Closing server due to timeout.")
    ...
    if packet_received:
        if invalid_dscp:
            result1 = "Failed"
            description1 = "Packet received with invalid DSCP"
        else:
            result1 = "Passed"
            description1 = "Success"
    else:
        result1 = "Failed"
        description1 = "Didn't received any packets"
    print(f"UDP IPv4 validation - {description1}")

    time.sleep(300)

    # ----------------- Validating UDP IPv6 packet ------------------------------
    PORT = 33334
    ...
    print(f"Starting packet capture...")
    filter_exp = f"ip6 and udp src port 22224 and udp dst port {PORT}"
    sniff_thread = threading.Thread(
        target=sniff,
        kwargs={
            "filter": filter_exp,
    ...
    if packet_received:
        if invalid_dscp:
            result2 = "Failed"
            description2 = "Packet received with invalid DSCP"
        else:
            result2 = "Passed"
            description2 = "Success"
    else:
        result2 = "Failed"
        description2 = "Didn't received any packets"

    print(f"UDP IPv6 validation - {description2}")
