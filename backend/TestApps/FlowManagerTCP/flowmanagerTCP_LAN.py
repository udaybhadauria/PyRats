    ...
    while attempt < max_retries:
        ...
        if response_code == 200 and "POST Request Successful" in response_body:
            ...
            if attempt < max_retries:
                ...
                result = "Failed"
                description = "Webcfg add for Device prioritization rule failed"
                write_test_result_to_json(mac_address, test_ID, result, description)
                sys.exit(1)

if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python3 flowmanagerTCP_LAN.py <test_ID> <mac_address> <webserver_ipv4> <webserver_ipv6>")
        sys.exit(1)
    ...
    # -------------------- IPv4 TCP connection validation ----------------------------
    ...
    ipv4_connected = True
    print("Validating TCP IPv4")
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind(("0.0.0.0", SRC_PORT))  # Bind source port
    try:
        sock.connect((SERVER_IPv4, DST_PORT))  # Connect to server
        print(f"Connected to {SERVER_IPv4}:{DST_PORT}")

        value = f"FlowManager%7Csrcip={src_ip}%7Cdstip={SERVER_IPv4}%7Cprotocol=tcp%7Csrcport={SRC_PORT}%7Cdstport={DST_PORT}%7Ccount={PACKET_COUNT}%7Cdscp={dscp}"
        print(f"input - {value}")
        ...
        result1 = "Failed"
        description1 = f"Connection failed:{e}"
    ...
    if ipv4_connected:
        ...
        while attempt < retry_count:
            ...
            if FMresult not in ["Passed", "Failed"]:
                ...
                break
    print(f"TCP IPv4 description: {description1}")
    time.sleep(60)

    # -------------------------------------------------------------------------------------------------

    # -------------------- IPv6 TCP connection validation ---------------------------------------------
    ...
    ipv6_connected = True
    print("Validating TCP IPv6")
    # Create IPv6 socket
    sock = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
    sock.bind(("::", SRC_PORT))

    try:
        sock.connect((SERVER_IPv6, DST_PORT, 0, 0))
        print(f"Connected to [{SERVER_IPv6}]:{DST_PORT}")

        value = f"FlowManager%7Csrcip={src_ip6}%7Cdstip={dst_ip6}%7Cprotocol=tcp%7Csrcport={SRC_PORT}%7Cdstport={DST_PORT}%7Ccount={PACKET_COUNT}%7Cdscp={dscp}"
        print(f"input - {value}")
        res = webpa_set(runParam, value, 0)
        if not res:
            sock.close()
        ...
        print("Connection failed:", e)
        result2 = "Failed"
        description2 = f"Connection failed:{e}"
    ...
    if ipv6_connected:
        ...
        while attempt < retry_count:
            ...
            if FMresult not in ["Passed", "Failed"]:
                ...
                break
    print(f"TCP IPv6 description: {description2}")
    # ---------------------------------------------------------------------------------------------------------
