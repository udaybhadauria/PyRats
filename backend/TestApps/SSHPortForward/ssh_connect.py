...
def fetch_row_id():
    ...
    for attempt in range(max_retries):
        ...
        if response_code == 200:
            ...
            print(f"Failed to fetch row_id, attempt {attempt + 1}/{max_retries}")
            if attempt < max_retries - 1:
...
def ssh_connect(ip_address):
    try:
        ...
        client.connect(ip_address, 25634, 'testuser', 'testuser@123', timeout=15, look_for_keys=False, allow_agent=False)
        ...
        description = f"Failed to connect to SSH service: {str(e)}"
...
if __name__ == "__main__":
    ...
    required_python_packages = ['paramiko']
    ...
    print("[INFO] Testing access to client SSH service via PortForward")
    result, description = ssh_connect(gateway_ip)
    write_test_result_to_json(mac_address, test_ID, result, description)
